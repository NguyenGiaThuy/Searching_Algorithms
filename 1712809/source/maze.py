...
class Node:
    def __init__(self, coordinate, parent = None, next_move_cost = 1, next_actual_cost = 1):
        ...
        if self.parent != None:
            ...
            self.actual_cost = self.parent.actual_cost + next_actual_cost
...
class Maze:
        ...
        if self.matrix[x][y] != 'x':
            ...
            for waypoint in self.waypoints:
                if waypoint[0] == x and waypoint[1] == y: 
                    ...
                    node1 = Node((x, y), current_node, next_move_cost, 1)
                    self.visited_coordinates.append(node1.coordinate)
                    node2 = Node((waypoint[2], waypoint[3]), node1, 0, 0) # Teleportation cost = 0
                    break
                ...
                if waypoint[2] == x and waypoint[3] == y: 
                    ...
                    node1 = Node((x, y), current_node, next_move_cost, 1)
                    self.visited_coordinates.append(node1.coordinate)
                    node2 = Node((waypoint[0], waypoint[1]), node1, 0, 0) # Teleportation cost = 0
                    break
    ...
    def visualize(self, sub_directory = '', file_name = ''):
        ...
        util.visualize_maze(self.matrix, self.bonuses, self.waypoints, self.start_coordinate, self.end_coordinate, self.path, self.path_cost, self.visited_coordinates, sub_directory, file_name)
